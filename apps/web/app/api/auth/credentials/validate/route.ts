...
export async function POST(request: NextRequest) {
  try {
    ...
    if (universityId !== 'bgu') {
      return NextResponse.json(
        { error: 'כרגע תמיכה רק באוניברסיטת בן-גוריון' },
        { status: 400 }
      );
    }
    ...
    if (!validationResult.success) {
      ...
      return NextResponse.json(
        { 
          ...
          authenticationFlow: credentialStatus.hasCredentials ? 'existing_user_failed' : 'new_user_failed',
    ...
    console.log(`✅ Smart validation completed successfully for ${userEmail} in ${responseTimeMs}ms`);

    return NextResponse.json({
      success: true,
      authenticationFlow: credentialStatus.hasCredentials ? 'existing_user_updated' : 'new_user_success',
  ...
  try {
    ...
    try {
      ...
      try {
        ...
        return { hasCredentials: false, hasValidCredentials: false, needsRevalidation: true, username: null };
      }
    }
 ...
 */
async function performRealValidation(username: string, password: string, universityId: string): Promise<any> {
  ...
  return new Promise((resolve, reject) => {
    ...
    const pythonProcess = spawn('python3', [
      ...
      '--username', username,
      '--password', password,
      '--fast-mode', 'true' // Enable fast mode for performance
    ], {
      cwd: scraperPath,
 ...
 */
async function saveValidatedCredentials(userEmail: string, universityId: string, username: string, password: string) {
  try {
    ...
    try {
      const { data, error } = await supabase
        .rpc('save_validated_credentials', {
          user_email_param: userEmail,
          university_id_param: universityId,
