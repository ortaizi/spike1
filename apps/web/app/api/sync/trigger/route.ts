...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!session?.user?.isDualStageComplete) {
      return NextResponse.json({ error: 'נדרש אימות דו-שלבי מלא לביצוע סנכרון' }, { status: 401 });
    }
    ...
    try {
      ...
      console.log(
        `✅ Sync started successfully for user ${session.user.email}, job ID: ${syncResult.jobId}`
      );
