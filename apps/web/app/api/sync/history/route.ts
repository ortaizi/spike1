...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!session?.user) {
      return NextResponse.json({ error: 'נדרש אימות למערכת' }, { status: 401 });
    }
    ...
    console.log(
      `📊 Getting sync history for user ${session.user.email}, limit: ${limit}, offset: ${offset}`
    );
    ...
    if (error) {
      ...
      return NextResponse.json({ error: 'שגיאה בטעינת היסטוריית הסנכרון' }, { status: 500 });
    }
...
export async function DELETE(_request: NextRequest) {
  try {
    ...
    if (error) {
      ...
      return NextResponse.json({ error: 'שגיאה במחיקת היסטוריית הסנכרון' }, { status: 500 });
    }
...
function getDefaultStatusMessage(status: string): string {
  switch (status) {
    case 'pending':
      return 'ממתין בתור לעיבוד';
    case 'running':
      return 'תהליך סנכרון פעיל';
    case 'creating_tables':
      return 'מכין מבני נתונים';
    case 'fetching_courses':
      return 'אוסף נתוני קורסים';
    case 'analyzing_content':
      return 'מנתח תוכן קורסים';
    case 'classifying_data':
      return 'מסווג ומארגן נתונים';
    case 'saving_to_database':
      return 'שומר נתונים במאגר';
    case 'completed':
      return 'סנכרון הושלם בהצלחה';
    case 'error':
      return 'תקלה בתהליך הסנכרון';
    case 'cancelled':
      return 'תהליך בוטל';
    default:
