...
interface CourseAnalysisResult {
  ...
  content: {
    ...
    files?: CourseContentItem[];
    other?: CourseContentItem[];
    [key: string]: CourseContentItem[] | undefined;
  };
  metadata: {
    total_items: number;
...
export function CourseAnalyzer({ userId, onAnalysisComplete }: CourseAnalyzerProps) {
  ...
  const updateStats = useCallback(() => {
    const totalItems = analysisResults.reduce(
      (sum, result) => sum + result.metadata.total_items,
      0
    );
    const avgConfidence =
      analysisResults.reduce((sum, result) => sum + result.metadata.analysis_confidence, 0) /
  ...
  const checkAutoAnalysisStatus = useCallback(async () => {
    try {
  ...
  const checkActiveSyncJob = useCallback(async () => {
    try {
  ...
  useEffect(() => {
    ...
    checkActiveSyncJob();
  }, [userId, loadExistingAnalyses, checkAutoAnalysisStatus, checkActiveSyncJob]);
              ...
              <Card key={result.course_id}>
                <CardHeader>
                  <CardTitle className='flex items-center justify-between'>
                    {result.course_name}
                    <Badge variant='outline'>{result.metadata.total_items} פריטים</Badge>
                  </CardTitle>
                  <CardDescription>
                ...
                <CardContent>
                  <div className='grid grid-cols-2 gap-4 md:grid-cols-4'>
                    {Object.entries(result.metadata.content_type_distribution).map(
                      ([type, count]) => (
              ...
              <Card key={result.course_id}>
                ...
                <CardContent>
                  <div className='space-y-4'>
                    <div>
                      <h4 className='mb-2 font-medium'>התפלגות תוכן</h4>
                    ...
                    <div>
                      <h4 className='mb-2 font-medium'>סטטיסטיקות</h4>
                      <div className='grid grid-cols-2 gap-4'>
                        <div>
                          <div className='text-muted-foreground text-sm'>זמן עיבוד</div>
                          <div className='text-lg font-bold'>
                            {result.metadata.processing_time.toFixed(2)} שניות
                          </div>
                        </div>
                        <div>
                          <div className='text-muted-foreground text-sm'>רמת דיוק</div>
                          <div className='text-lg font-bold'>
                            {(result.metadata.analysis_confidence * 100).toFixed(1)}%
                          </div>
                        </div>
                      </div>
                    </div>
